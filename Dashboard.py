...
# Proteção contra erros de conexão e JSON vazio
if response.status_code != 200:
    st.error(f"Erro ao conectar com a API: Código {response.status_code}")
    st.stop()

try:
    dados = pd.DataFrame.from_dict(response.json())
except Exception:
    st.warning("Nenhum dado foi encontrado para a combinação de filtros selecionada.")
    st.stop()
...
with aba2:
    ...
    with coluna1:
        ...
        st.plotly_chart(fig_vendas_estados, width="stretch")
    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
...
with aba3:
    ...
    with coluna1:
        ...
        st.plotly_chart(fig_receita_vendedores, width="stretch")
    with coluna2:
        st.metric('Quantidade de vendas', formata_numero(dados.shape[0]))
        ...
        st.plotly_chart(fig_vendas_vendedores, width="stretch")